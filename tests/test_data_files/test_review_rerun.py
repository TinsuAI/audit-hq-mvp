...
def _upload_and_parse(client, app_db: AppDb) -> tuple[int, dict]:
...
def _seed_findings_confirmed(app_db: AppDb, company_code: str) -> None:
...
def test_edit_column_reruns_only_affected(app_db: AppDb):
...
def test_edit_key_column_reruns_balance_check(app_db: AppDb):
...
def test_no_column_change_does_not_rerun(app_db: AppDb):
    client = TestClient(app)
    _login(client)
    _company(app_db)
    fid, base_map = _upload_and_parse(client, app_db)
