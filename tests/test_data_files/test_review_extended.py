...
def _extended_m15_bytes() -> bytes:
    wb = Workbook()
    ws = wb.active
    ws.title = "BCQT_NPL"
    for _ in range(5):
        ws.append([None] * len(_NUMBERING))
    ws.append(_NUMBERING)
    for i in range(3):
        # 10 + (40+60) - 0 - 0 - 80 - 0 = 30 → đẳng thức khớp.
        ws.append([i + 1, "ACC", f"MAT{i}", None, 10, 40, 60, 0, 0, 80, 0, 30])
    buf = io.BytesIO()
    wb.save(buf)
    return buf.getvalue()
...
def _upload(client: TestClient) -> None:
    r = client.post(
        ...
        data={"year": "2024"},
        files={"m15": ("Mau15_NVL.xlsx", _extended_m15_bytes(), XLSX_MIME)},
