...
# tức cổng xác nhận cột bật. Giống fixture của `test_review_screen.py`.
_HEADER = [
...
def _unreadable_m15_bytes() -> bytes:
    """Workbook không trang nào khớp biểu — chèn một cột đầu và bỏ khối tiêu đề."""
    wb = Workbook()
    ws = wb.active
    ws.title = "Bìa"
    ws.append(["Trang bìa"])
    ws2 = wb.create_sheet("Dữ liệu")
    ws2.append(["Nguồn", *_HEADER])
    for i in range(3):
...
def _awaiting_confirm(app_db: AppDb) -> tuple[TestClient, int, dict]:
    ...
    upload_and_ingest(client, _CODE, "Mau15_NVL.xlsx", _m15_bytes())
    drain_jobs()
    with app_db.SessionLocal() as db:
...
def test_reingest_of_an_unreadable_file_also_returns_to_the_file_page(app_db: AppDb):
    ...
    client = _client(app_db)
    upload_and_ingest(client, _CODE, "Mau15_NVL.xlsx", _unreadable_m15_bytes())
    drain_jobs()
    with app_db.SessionLocal() as db:
        c = db.query(Company).filter_by(code=_CODE).one()
...
def test_a_rejected_column_index_returns_to_the_file_page_with_the_reason(app_db: AppDb):
    ...
    assert r.status_code == 303
    assert r.headers["location"].startswith(f"{file_page_url(_CODE, fid)}?error=")
    page = client.get(r.headers["location"]).text
    assert "không phải chỉ số cột" in page
...
def test_file_page_prints_the_queued_ingest_and_polls_for_itself(app_db: AppDb):
    ...
    assert f"/documents/ingest.json?year={_YEAR}&amp;file={fid}" in page
...
def test_status_endpoint_reloads_the_file_page_when_the_file_page_asks(app_db: AppDb):
    ...
    from_file = client.get(
        f"/companies/{_CODE}/documents/ingest.json?year={_YEAR}&file={fid}"
    ).json()
...
def test_status_endpoint_rejects_a_file_of_another_company(app_db: AppDb):
    ...
    r = client.get(f"/companies/DN_KHAC/documents/ingest.json?year={_YEAR}&file={fid}")
...
def test_file_page_says_the_checks_have_not_run_and_puts_the_action_beside_it(
    app_db: AppDb,
):
    ...
    assert "chưa chạy kiểm tra lần nào" in page
    block = page.split('class="fp-checks"', 1)[1].split("</div>", 1)[0]
    assert f'action="/companies/{_CODE}/run-checks"' in block


def test_period_row_says_the_checks_have_not_run_beside_its_own_run_button(
    app_db: AppDb,
):
    ...
    block = page.split('class="ds-run-checks"', 1)[1].split("</div>", 1)[0]
    assert "Kiểm tra chưa chạy lần nào" in block
    assert f'action="/companies/{_CODE}/run-checks"' in block
...
def test_both_screens_go_quiet_once_the_checks_have_run(app_db: AppDb):
    ...
    assert "chưa chạy kiểm tra lần nào" not in file_page
    assert "Kiểm tra chưa chạy lần nào" not in period_page


# ───────────────────── nhãn nút ─────────────────────

def test_primary_button_names_the_two_things_it_does_without_an_emoji(app_db: AppDb):
    client, fid, _ = _awaiting_confirm(app_db)

    page = client.get(file_page_url(_CODE, fid)).text

    label = page.split('class="btn btn-primary">', 1)[1].split("<", 1)[0]
    assert "Xác nhận vị trí cột" in label
    assert f"nạp lại dữ liệu kỳ {_YEAR}" in label
    assert not any(ch for ch in label if ord(ch) > 0x2100), label
